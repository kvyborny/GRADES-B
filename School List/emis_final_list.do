...
import delimited "$data\Dataset\osrm\DS_longlist_for_PMU.csv", clear
    ...
    Not matched                        10,105
        ...
        from using                     10,097  (_merge==2)

    Matched                             5,265  (_merge==3)
    -----------------------------------------
	*/
...
import delimited "$data\Dataset\osrm\TE_longlist_high_conf_for_PMU.csv", clear	//cleaned the TE list to match the vars for appending
    ...
    Not matched                        13,642
        ...
        from using                     13,642  (_merge==2)

    Matched                             1,720  (_merge==3)
    -----------------------------------------
	*/
	...
	*/
    ...
    -----------------------------------------
    Not matched                         8,386
        from master                     8,379  (_merge==1)
        from using                          7  (_merge==2)

    Matched                             6,986  (_merge==3)
    -----------------------------------------
	*/
	keep if _m == 3 | _m ==2
	drop _m
