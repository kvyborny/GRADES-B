...
***merging to get some schools functional status
	
	drop if FunctionalStatus == "Non-Functional"
	
	import excel "$data\Admin Data\EMIS Data\GRADES longlist 20260122.xlsx", sheet("Sheet1") firstrow clear
	
	destring Emiscode, replace force
	rename Emiscode EMISCode
	rename FunctionalStatus FunctionalStatus_
	keep EMISCode FunctionalStatus
    ...
    Matched                             6,993  (_merge==3)
    -----------------------------------------
	*/
	
	tab FunctionalStatus, m
	tab FunctionalStatus FunctionalStatus_, m
	replace FunctionalStatus=FunctionalStatus_ if FunctionalStatus_ !="" // updating functional status with what was received by PMU
	tab FunctionalStatus FunctionalStatus_ , m
	drop FunctionalStatus_
	tab FunctionalStatus, m
	drop _merge
...
*/
	gen STEPB = 1 if _m==3
	replace STEPB = 0 if _m==2
	drop _m 

	tempfile emis_clean3																	// dropped Step-B schools from EMIS
	sa `emis_clean3'
	
***marking BHCIP schools

import excel "$data\Admin Data\BHCIP\BHCIP_edited.xlsx", sheet("BHCIP 5 Districts Focused schoo") firstrow clear

	drop if BEMIS ==.
	ren BEMIS EMISCode 
	keep EMISCode Lati Long
	
	merge 1:1 EMISCode using `emis_clean3'
	
	/*  Result                      Number of obs
    -----------------------------------------
    Not matched                        14,560
        from master                         4  (_merge==1)
        from using                     14,556  (_merge==2)

    Matched                               282  (_merge==3)
    -----------------------------------------

*/
	
	gen BHCIP = 1 if _m ==3 | _m == 1
	replace BHCIP = 0 if _m == 2
	
	replace Longitude = Long if _m == 1										// There are 4 schools in BHCIP which do not have coordinates in EMIS or RTSM so we are using the coordinates from the same file
