...
import excel "$data\Admin Data\EMIS Data\Balochistan EMIS Flatsheet 2024-25.xlsx", sheet("16-8-25") firstrow clear
	...
	*Enrollment from Kachi - Grade 12
	...
	SubTehsil UC VillageName Genderupdated SchoolLevel FunctionalStatus girls_enrol_p boys_enrol_p girls_enrol_total boys_enrol_total TotalRooms enrol_per_room SpaceForNewRooms SchoolOwnedBy girls_prop_p boys_prop_p girls_prop_total boys_prop_total total_enrollment TeachingMale TeachingFemale TotalTeachingStaff overcrowded // keeping only relevant vars
...
***Using Survey Auto's school coordinates
	...
	*/
	...
	keep BEMIS XCord YCord Level Gender schoolname
...
***SchoolName 
	replace SchoolName = schoolname if SchoolName ==""							//Taking schoolname from RTSM of those which are not in the EMIS  
	drop schoolname
...
***Tagging already assessed schools
	...
	gen schoolname = substr(SchoolName, strpos (SchoolName, "-") + 1, .)
	
	
	keep EMISCode _GPSCoordinates_latitude _GPSCoordinates_longitude schoolname schoollevel
	...
	*/
	...
	replace SchoolName = schoolname if SchoolName == ""							//Getting schoolname of 10 schools which are not in EMIS but are in already assessed schools
	drop schoolname	
...
****Cleaning school level variable
	replace SchoolLevel = schoollevel if SchoolLevel ==""
	replace Level = SchoolLevel if Level ==""
