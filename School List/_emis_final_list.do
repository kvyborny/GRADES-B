...
*******************************************************************************
	 ...
	 merge 1:1 EMISCode using  "$data\Dataset\admin\EMIS\emis_final_clean.dta", keepusing (SchoolName Latitude Longitude gender Level /*STEPB BHCIP*/)
	 /*
...
*	drop STEPB BHCIP
	...
	export delimited using "$data\Dataset\admin\Datasets prepared to share with PMU\overcrowded_girls_schools.csv", replace // list of overcrowded_girls_schools
...
*******************************************************************************
	use "$data\Dataset\admin\EMIS\emis_final_clean.dta", clear
...
******************************************************************************
	...
	*/
	...
	intervention_ece intervention_dste Shelterless nearby_non_functional non_func_schname non_func_emis girls* boys*
...
******************************************************************************
	...
	import excel "$data\Admin Data\GRADES B\Round 1\GRADES Latest sheet 3278 schools (2).xlsx", sheet("Sheet1") firstrow clear
	
	* Rename girls enrollment
	rename (BP BQ BR BS BT BU BW BX BY CA CB CC CD) (girls_kachi girls_pakki  girls_2 girls_g3 girls_g4 girls_g5 girls_g6 girls_g7 girls_g8 girls_g9 girls_g10 girls_g11 girls_g12 )

	* Rename boys enrollment
	rename (Kachi Class1 Class2 Class3 Class4 Class5 Class6 Class7 Class8 Class9 Class10 Class11 Class12) (boys_kachi boys_pakki  boys_g2 boys_g3 boys_g4 boys_g5 boys_g6 boys_g7 boys_g8 boys_g9 boys_g10 boys_g11 boys_g12)
	...
	keep District SchoolName emiscode Level gender Location ECE Shelterless Latitude Longitude Doubleshift UC Tehsil girls* boys*
...
*	drop intervention_dste intervention_ece master shelterless ECE Doubleshift tag
	...
	* Standardize UC names
	...
	replace UC = "BADAI" if inlist(UC,"BADAHI","BADAI")
	
	replace UC = "BEDAR UNDROON" if inlist(UC,"BEDAR ANDRON","BEDAR UNDROON")
	
	replace UC = "FAIZABAD JAMALI" if inlist(UC,"FAIZA ABAD JAMALI","FAIZABAD JAMALI")
	
	replace UC = "GANDAR" if inlist(UC,"GANDHAR","GANDAR")

	replace UC = "LAKHORIAN" if inlist(UC,"LAKORIAN","LAKHORIAN")
	
	replace UC = "NAAG" if inlist(UC,"NAG","NAAG")
	
	replace UC = "NARAZAI" if inlist(UC,"NARAIZAI","NARAZAI", "NAREZAI")
	
	replace UC = "NASIRABAD" if inlist(UC,"NASEER ABAD")

	replace UC = "SHORI DRABI ANDROON" if inlist(UC,"SHORI DHARBI ANDROON")
	
	replace UC = "TAKRA" if inlist(UC,"TAKHRA")
	
	replace UC = "TOMNI" if inlist(UC,"TOMANI")
	
	replace UC = "TUKHOI" if inlist(UC,"TUKHAI")
	
	replace UC = "YARU-I" if inlist(UC,"YAR0-1")
	...
	gen drop =.
	encode intervention, gen (interv )
	drop intervention
	ren interv intervention
	gen interv_DS=1 if inlist(intervention, 1,2,3)
	replace drop = 1 if master ==1 & assessment_done ==1 & interv_DS==1 & Doubleshift==2 & nearby_non_functional !="Yes"
	*drop if drop==1
	*keep if master==1 | (assessment_done ==1 & intervention ==4 & master!=1)
*	keep if master ==1 | intervention ==4
	gen DS_TE=.
	tab intervention
	replace DS_TE=1 if inlist(intervention, 1,2,3,4)
	replace DS_TE=2 if inlist(intervention, 7,8,9)
	...
	*estpost tab UC intervention ///
	...
	save "$data\Dataset\admin\Longlist\long_list.dta", replace							//saving .dta file 
	export delimited using "$data\Dataset\admin\Longlist\long_list.csv", replace			//saving the csv file
/*	
	preserve
	gen count = 1
